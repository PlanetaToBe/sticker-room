...
function onDeviceMotionChangeEvent(evt) {
	deviceMotionAcc = evt.acceleration;
	deviceMotionInterval = evt.interval;
	//console.log("accX: " + roundTo2(deviceMotionAcc.x) + ", accY: " + roundTo2(deviceMotionAcc.y) + ", accZ: " + roundTo2(deviceMotionAcc.z));
}
...
THREE.DeviceControls = function ( camera, worldCenter ) {
	...
	this.accelerationForce = new THREE.Vector3();
	...
	this.calQ = function() {
		...
		if (this.autoAlign && this.alpha !== 0) {
	 	...
	 	//
	 	scope.accelerationForce.x = deviceMotionAcc.x * deviceAccelImplementationFix;
	 	scope.accelerationForce.y = deviceMotionAcc.y * deviceAccelImplementationFix;
	 	scope.accelerationForce.z = deviceMotionAcc.z;
	 	//scope.accelerationForce.multiplyScalar(deviceMotionInterval);
	};
		...
		if(updateInterval > 110)
		{
			...
			if(trulyFullyStart){
				var msg = {
					...
					'quaZ': eyeFinalQ2._z,
					'quaW': eyeFinalQ2._w,
					'acceX': scope.accelerationForce.x,
					'acceY': scope.accelerationForce.y,
					'acceZ': scope.accelerationForce.z
				};
