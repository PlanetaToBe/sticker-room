...
THREE.DeviceControls = function ( camera, worldCenter ) {
	...
	*/
	...
	this.autoAlign = false;		// origin: true
	...
	this.finalQ3 = new THREE.Quaternion(); // from VRDisplay
	this.vrdisplayObject = new THREE.Object3D();
	...
	this.calQ = function() {
		...
		*/
	 	...
	 	// V.2 - VRControls with getting VRDisplay data
	 	if (vrDisplay)
	 	{
	 		var pose;
	 		...
	 		if ( pose.orientation !== null )
	 		{
	 			...
	 			this.finalQ3.fromArray( pose.orientation );
	 			this.vrdisplayObject.quaternion.fromArray( pose.orientation );

	 			this.finalQ.copy( this.finalQ3 );
	 		}

	 		if ( pose.position !== null )
	 		{
	 			this.vrdisplayObject.position.fromArray( pose.position );
	 		}
	 		...
	 		if ( this.standing )
	 		{
	 			...
	 			{
					this.vrdisplayObject.updateMatrix();

					standingMatrix.fromArray( vrDisplay.stageParameters.sittingToStandingTransform );
					this.vrdisplayObject.applyMatrix( standingMatrix );
				}
				else {
					this.vrdisplayObject.position.setY( this.vrdisplayObject.position.y + this.userHeight );
				}
	 		}

	 		this.vrdisplayObject.position.multiplyScalar( scope.scale );
	 	}
	};
